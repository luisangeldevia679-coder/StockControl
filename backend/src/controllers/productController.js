...
const Product = require('../models/product'); // Importamos el nuevo modelo

 tempProductsDb = []; // Tu lista temporal de la base de datos

 createProduct = (req, res, next) => {
    try {
        const { name, price, stock, supplierId } = req.body;

        // Instanciamos un nuevo producto usando el plano del modelo
        ...
        res.status(201).json({
            success: true,
            message: "Producto registrado bajo el modelo oficial",
            data: newProduct
        });
    } catch (error) {
        next(error);
...
const { tempSuppliersDb } = require('./supplierController'); // Importas la lista de proveedores

// ... dentro de la función createProduct, antes de hacer el push:
const supplierExists = tempSuppliersDb.find(s => s.id === Number(supplierId));
if (!supplierExists) {
    return res.status(404).json({ 
        success: false, 
        message: `El proveedor con ID ${supplierId} no existe en el sistema. Regístralo primero.` 
    });
}
